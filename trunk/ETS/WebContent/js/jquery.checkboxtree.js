 ...
 */
/*!
 * This is modified version to add feature of unchecking parent when all child nodes are unchecked.
 * Added one more option for onUncheck.ancestors as uncheckIfAll, to uncheck parent node when all child nodes are unchecked.  
 * Change No : 1234
 * Modified by: Punitkumar Damaniya
 */
...
$.widget("daredevel.checkboxTree", {
 ...
 /** Change 1234, by Punitkumar Damaniya Start */       	
        if(this.options.onUncheck.ancestors == 'uncheckIfAll')
    	{
        	var parentNode=this._parentNode(li)
        	while(parentNode.length>0)
        	{
                if(jQuery('> ul > li input:checkbox:not(:checked)',parentNode).length==jQuery('> ul > li',parentNode).length)
            	{
                	parentNode.find("> input:checkbox:first").prop("checked",false);
                	
                    if (this.options.onUncheck.node == 'expand') {
                    	this.expand(parentNode);
                    }
                    else  if (this.options.onUncheck.node == 'collapse') {
        ...
        /** Change 1234, by Punitkumar Damaniya End  */        
